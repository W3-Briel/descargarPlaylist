...
class Video():
    ...
    def descargar(self):
        try:
            ...
            self.audio.download(output_path=self.path)
            self.path = f"{self.path}{self.get_title()}.mp4"
            print("\n")
            print("*"*30)
...
class Convertir():
    ...
    def convertir_todo_a_mp3(self):
        for archivo in self.list_obj:
            ...
            try:
                print(archivo.path)
                pydub.AudioSegment.from_file(f"{archivo.path}","mp4").export(f"./descargas_mp3/audio_{archivo.get_title()}.mp3","mp3")
                remove(archivo.path)
                print(f"{archivo.get_title()} completo!!...")
            except:
                print(f"error {archivo.get_path()}")
                print(f"list dir; {listdir("./descargas_mp3")}")